...
class Package():
    ...
    def __init__(self, to, data, personality = 1):
        self.TO = to #string IP
        if type(data) == list:
            self.data = data #list (choose, args*)
            for i in range(len(self.data)):#whitespaces
    ...
    def __repr__(self):
        return "TO: "+str(self.TO)+" - DATA: "+str(self.data)+" - FROM: "+str(self.FROM)+" - PERSONALITY: "+str(self.personality)+" - ERRCODE: "+str(self.errcode)
